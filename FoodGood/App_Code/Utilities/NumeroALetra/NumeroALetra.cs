...
{
    /// <summary>
    /// Summary description for NumeroALetra
    /// </summary>
    public class NumeroALetra
    {
        ...
        public static string DeleteIdFromStringList(string idList, string idToDelete)
        {
            if (string.IsNullOrEmpty(idList))
                return "";
            string[] list = idList.Split(new char[] { ',' });
            StringBuilder result = new StringBuilder();
        ...
        public static string GetWebsiteRoot()
        {
            ...
            return HttpUtility.UrlDecode(wsroot);
        }

        public static int GetDifferenceBetweenDates(DateTime fechaDesde, DateTime fechaHasta, string tipo)
        {
            if (string.IsNullOrEmpty(tipo) || fechaDesde == null || fechaHasta == null)
                return 0;

            //years
            TimeSpan diff = fechaHasta - fechaDesde;
            int Years = diff.Days / 366;
            DateTime workingDate = fechaDesde.AddYears(Years);

            while (workingDate.AddYears(1) <= fechaHasta)
            {
            ...
            //months
            diff = fechaHasta - workingDate;
            int Months = diff.Days / 31;
            workingDate = workingDate.AddMonths(Months);

            while (workingDate.AddMonths(1) <= fechaHasta)
            {
