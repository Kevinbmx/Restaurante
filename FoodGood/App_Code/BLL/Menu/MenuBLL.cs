...
namespace FoodGood.Menu.BLL
{
    ...
    public class MenuBLL
    {
        ...
        {
            string xmlMenu = "";
            using (TextReader tr = new StreamReader(HttpContext.Current.Request.MapPath("~/Administracion/DataFile/Menu.xml")))
            {
        ...
        {
            string[] menuClassLevel = { "nav metismenu", "nav nav-second-level", "nav nav-third-level", "nav nav-fourth-level", "nav nav-fifth-level" };
            ...
            foreach (Menu objMenu in theMenus)
            {
                menuHtml.Append("<li>");
                ...
                string isFirstLevel = (menuLevel == 0 ? "<span class=\"nav-label\">" + objMenu.Text + "</span>" : objMenu.Text);
                string hasChilds = (objMenu.SubMenus.Count > 0 ? "<span class=\"fa arrow\"></span>" : "");
